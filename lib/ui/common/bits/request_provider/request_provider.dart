...
import 'package:flutter_dasher/ui/common/bits/request_provider/request_state.dart';
...
abstract class RequestProvider<Value> with ChangeNotifier, NetworkLoggy {
  ...
  bool isLoading() {
    return state.maybeMap(loading: (_) => true, orElse: () => false);
  }
  ...
  Future<void> executeRequest({
    ...
    try {
      _state = _requestState.maybeMap(
        success: (r) => RequestState.loading(resultMaybe: r.value),
